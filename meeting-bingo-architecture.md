...
graph TB
    subgraph Browser["Browser Environment"]
        subgraph React["React Application"]
            ...
            GL --> CS
            SR --> CS
            CS["Context State"] --> LS["localStorage\nPersistence"]
        end

        subgraph BrowserAPIs["Browser APIs (Built-in, Free)"]
            WSA["Web Speech API"]
            AC["Audio Context"]
            CB["Clipboard API"]
            SH["Share API"]
...
```mermaid
flowchart LR
    subgraph CardGen["1. Card Generation"]
        direction LR
        A1[Select Category] --> A2[Shuffle Words] --> A3[Pick 24 Words] --> A4[Render Card]
    end

    subgraph SpeechFlow["2. Speech Recognition"]
        direction LR
        B1[Audio Input] --> B2[Speech API] --> B3[Transcript Text] --> B4[Word Detection]
        B4 --> B5{Match Found?}
        B5 -->|Yes| B6[Fill Square] --> B7[Update UI]
        B5 -->|No| B4
    end

    subgraph WinFlow["3. Win Detection"]
        direction LR
        C1[Square Filled] --> C2[Check Lines] --> C3{BINGO?}
        C3 -->|Yes| C4[Celebrate & Share]
        C3 -->|No| C1
    end
```
...
```mermaid
graph TD
    Players["👥 Players"] --> OptionA & OptionB & OptionC

    OptionA["Option A — WebSocket Server\n(Requires Backend)"]
    OptionA --- A1["✅ Real-time sync across players"]
    OptionA --- A2["✅ Shared game state"]
    OptionA --- A3["✅ Live leaderboard"]
    OptionA --- A4["💰 $5–20/month hosting"]

    OptionB["Option B — Peer-to-Peer\n(WebRTC)"]
    OptionB --- B1["✅ No backend required"]
    OptionB --- B2["✅ Direct browser connections"]
    OptionB --- B3["⚠️ Limited to ~10 players"]
    OptionB --- B4["🆓 Free"]

    OptionC["Option C — Firebase\nRealtime Database"]
    OptionC --- C1["✅ Managed real-time sync"]
    OptionC --- C2["✅ Easy implementation"]
    OptionC --- C3["✅ Generous free tier"]
    OptionC --- C4["🆓 Free up to 100 concurrent"]
```
